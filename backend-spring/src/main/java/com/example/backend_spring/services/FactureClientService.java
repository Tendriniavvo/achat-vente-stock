...
public class FactureClientService {
        ...
        public FactureClient genererFactureDepuisLivraison(int livraisonId, int utilisateurId) {
                ...
                List<LigneCommandeClient> lignesCommande = ligneCommandeClientRepository
                ...
                for (LigneLivraison ll : lignesLivraison) {
                        ...
                        LigneCommandeClient lcc = lignesCommande.stream()
                                        ...
                                        .findFirst()
                                        .orElseThrow(() -> new RuntimeException("Incohérence : L'article "
                                                        + ll.getArticle().getNom()
                ...
                journalAuditService.logAction(
                                ...
                                "Facture générée pour la livraison " + livraison.getReference() + ". Montant: "
                                                + montantTotalTotal);
        ...
        public FactureClient creerAvoir(int factureOriginaleId, String motif, int utilisateurId,
                ...
                if (lignesAvoir == null || lignesAvoir.isEmpty()) {
                        ...
                        List<LigneFactureClient> lignesOrig = ligneFactureClientRepository
                                        .findByFactureId(factureOriginaleId);
                        for (LigneFactureClient lfOrig : lignesOrig) {
                                ...
                                BigDecimal montantHT = lfOrig.getPrixUnitaire()
                                                .multiply(new BigDecimal(lfOrig.getQuantite()));
                                BigDecimal tauxTaxe = lfOrig.getTaxe() != null ? lfOrig.getTaxe().getTaux()
                                                : BigDecimal.ZERO;
                        ...
                        for (LigneAvoirRequest req : lignesAvoir) {
                                LigneFactureClient lfOrig = ligneFactureClientRepository
                                                .findById(req.getLigneFactureId())
                                                .orElseThrow(
                                                                () -> new RuntimeException(
                                                                                "Ligne de facture non trouvée: " + req
                                                                                                .getLigneFactureId()));

                                if (req.getQuantite() > lfOrig.getQuantite()) {
                                        throw new RuntimeException(
                                                        "La quantité de l'avoir ne peut pas dépasser la quantité facturée.");
                                }
                                ...
                                BigDecimal montantHT = lfOrig.getPrixUnitaire()
                                                .multiply(new BigDecimal(req.getQuantite()));
                                BigDecimal tauxTaxe = lfOrig.getTaxe() != null ? lfOrig.getTaxe().getTaux()
                                                : BigDecimal.ZERO;
                                BigDecimal montantTaxe = montantHT.multiply(tauxTaxe).divide(new BigDecimal("100"), 2,
                                                BigDecimal.ROUND_HALF_UP);
                ...
                journalAuditService.logAction(
                                ...
                                "Avoir créé pour la facture " + factureOrig.getReference() + ". Motif: " + motif
                                                + ". Montant: "
        ...
        public FactureClient validerFacture(int id, int utilisateurId) {
                ...
                List<JournalAudit> creationAudits = journalAuditRepository
                                .findByModuleAndReferenceObjetOrderByDateActionDesc(
                ...
                boolean isAdmin = utilisateur.getRoles().stream()
                                .anyMatch(r -> r.getNom().equalsIgnoreCase("Administrateur"));
                ...
                if (facture.getReference().startsWith("AVOIR-")) {
                        ...
                        List<JournalAudit> avoirAudits = journalAuditRepository
                                        .findByModuleAndReferenceObjetOrderByDateActionDesc(
                        ...
                        if (creationAvoirAudit.isPresent()
                                        && creationAvoirAudit.get().getUtilisateur().getId() == utilisateurId
                                        && !isAdmin) {
                                throw new RuntimeException(
                ...
                String actionLabel = facture.getReference().startsWith("AVOIR-") ? "VALIDATION_AVOIR"
                                : "VALIDATION_FACTURE";
